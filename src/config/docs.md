...
| checkpoint_compression_algorithm | Compression algorithm for hummock version checkpoint. | "zstd" |
| checkpoint_read_chunk_size | Chunk size in bytes for reading large checkpoints. Large checkpoints are read in parallel chunks to avoid single-request timeout issues. Default: 128MB | 134217728 |
| checkpoint_read_max_in_flight_chunks | Maximum number of concurrent chunk reads when reading large checkpoints. Higher values may improve read throughput but increase memory usage. Memory usage = `checkpoint_read_chunk_size` * `checkpoint_read_max_in_flight_chunks` Default: 4 | 4 |
...
| compaction_task_id_refill_capacity | The number of compaction task ids to prefetch from the meta store in one batch. | 64 |
...
| iceberg_compaction_config_refresh_interval_sec | Maximum time to reuse cached iceberg compaction schedule config before refreshing it from meta catalog. | 60 |
| iceberg_compaction_report_timeout_sec | Maximum time to wait for an iceberg compaction task report before the lease expires. | 1800 |
...
| iceberg_compaction_pull_interval_ms | Pull interval for iceberg compaction task requests in milliseconds. | 5000 |
...
| table_change_log_cache_capacity |  | 60 |
...
| join_hash_map_evict_interval_rows | Number of processed rows between periodic join cache evictions. Values smaller than 1 will be clamped to 1 by the executor. | 16 |
...
| Config | Description | Default |
|--------|-------------|---------|
| adaptive_parallelism_strategy | The strategy for Adaptive Parallelism. | "BOUNDED(64)" |
